...
**Read-only in the app:** mirrored rows are tagged with their tricount name and rendered read-only (edit/delete suppressed) in the expense list and the budget per-category drill-down — they're managed on the Sync tab, not hand-edited (a manual edit would be overwritten, a delete re-created, on the next sync). Detection is by `expense_id` membership in `tricount_entry_map` (precise — a manual expense that happens to sit in the Tricount category stays editable). Removal is explicit on the Sync tab: **Disconnect** drops the link and ledger but keeps the mirrored expenses as plain rows (history); **Unlink & delete** also removes every expense the link produced. A finished tricount can instead be paused (`is_active = false`) to freeze its rows.
