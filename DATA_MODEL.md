...
Regular categories may optionally carry a cap via `cap_amount` (JTBD #8). When configured, the expense form surfaces an explicit labelled control whenever a logged amount exceeds the cap, shown between the (labelled) primary and overflow budget bars: a short line stating how much is over the cap, followed by a row of tappable chips — one per allowance (icon + name) plus a "No cap" chip — with the active choice highlighted. Picking an allowance chip splits the expense into two sibling rows (cap → this category, overflow → that allowance) sharing a `split_group_id`; "No cap" logs a single row at the full amount. Caps are EUR-denominated regardless of the expense's input currency — for BRL entries the form compares `amount × exchange_rate` against `cap_amount`, and the resulting primary row stores `converted_amount = cap_amount` exactly while the original-currency `amount` absorbs sub-cent rounding. The overflow allowance defaults to the user's last pick (cached in localStorage under `qb:<household>:last_overflow`, persisted on submit and sticky across category changes), falling back to the first allowance for a fresh browser. A single `cap_amount > 0` row-level CHECK enforces positivity; there is no DB-level coupling between a category and a specific allowance.
