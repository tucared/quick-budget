...
Regular categories may optionally carry a cap via `cap_amount` (JTBD #8). When configured, the expense form surfaces an inline toggle whenever a logged amount exceeds the cap: ON splits the expense into two sibling rows (cap → this category, overflow → an allowance the user picks at log time) sharing a `split_group_id`, OFF logs a single row at the full amount. Caps are EUR-denominated regardless of the expense's input currency — for BRL entries the form compares `amount × exchange_rate` against `cap_amount`, and the resulting primary row stores `converted_amount = cap_amount` exactly while the original-currency `amount` absorbs sub-cent rounding. The overflow allowance defaults to the user's last pick (cached in localStorage under `qb:<household>:last_overflow`, sticky across category changes and submits), falling back to the first allowance for a fresh browser; pill buttons let the user switch when multiple allowances exist. A single `cap_amount > 0` row-level CHECK enforces positivity; there is no DB-level coupling between a category and a specific allowance.
