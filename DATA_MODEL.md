...
Regular categories may optionally carry a cap via `cap_amount` (JTBD #8). When configured, capping is controlled by **tapping the category tile** whenever a logged amount exceeds the cap: the first tap selects the category with the cap ON (split the expense into two sibling rows — cap → this category, overflow → an allowance — sharing a `split_group_id`), and re-tapping the already-selected tile cycles the overflow target through each allowance and then to a no-cap state (single row at the full amount), repeating. The selected tile shows the current state directly as a small corner badge: the overflow allowance's icon while capping, or a struck `€` for no-cap. Caps are EUR-denominated regardless of the expense's input currency — for BRL entries the form compares `amount × exchange_rate` against `cap_amount`, and the resulting primary row stores `converted_amount = cap_amount` exactly while the original-currency `amount` absorbs sub-cent rounding. The overflow allowance defaults to the user's last pick (cached in localStorage under `qb:<household>:last_overflow`, persisted on submit and sticky across category changes), falling back to the first allowance for a fresh browser. A single `cap_amount > 0` row-level CHECK enforces positivity; there is no DB-level coupling between a category and a specific allowance.
