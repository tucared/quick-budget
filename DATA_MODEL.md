...
Regular categories may optionally carry a cap via `cap_amount` + `overflow_category_id` (JTBD #8). When configured, the expense form surfaces an inline toggle whenever a logged amount exceeds the cap: ON splits the expense into two sibling rows (cap → this category, overflow → the linked allowance) sharing a `split_group_id`, OFF logs a single row at the full amount. Caps are EUR-denominated regardless of the expense's input currency — for BRL entries the form compares `amount × exchange_rate` against `cap_amount`, and the resulting primary row stores `converted_amount = cap_amount` exactly while the original-currency `amount` absorbs sub-cent rounding. Two row-level CHECK constraints enforce that `cap_amount` + `overflow_category_id` are both-set-or-both-null and that a category cannot overflow into itself; cross-row guards ("overflow target must be an allowance in the same household") are enforced only by the seed today — admin UI is deferred.
