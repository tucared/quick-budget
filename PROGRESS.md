...
When I discover the app, I want to create a household, set our currencies, and invite my partner myself, so we can both be logging expenses without anyone provisioning accounts for us. Self-service signup lives at `/signup` (linked from `/login` as "Sign up"): the founder enters their email + password, optional name, and (unless the email turns out to be invited — see below) household name, base/secondary currencies, and optional partner email(s) (up to 10), and Supabase's public `signUp` plus the `handle_new_user()` DB trigger create the household, seed a default starter set of categories (6 spending + the founder's personal allowance), and pre-authorize each partner email. As the visitor types their email, a debounced check against `public.check_pending_invite` collapses the household-setup fields automatically if that address is already invited, rather than showing them greyed out with a static note. A partner later signs up with their own email and is auto-joined to the same household (routed by the invite, no duplicate household), getting a personal allowance named from their own signup name (or its email local-part, since the name field is optional). Email confirmation is required and is the materialization point — the household/join is created when the confirmation link (which reuses `/auth/callback`) is clicked, so abandoned signups leave no household behind. There is no service-role key, so this trigger-routed public signup *is* the multi-member onboarding path. Category and household management UIs are deferred — see DATA_MODEL.md decision #1 for the `household_invites` table, trigger branching, and the live invite check.
