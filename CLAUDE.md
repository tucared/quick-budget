...
- **`gh` CLI is available** for reading GitHub Actions logs — the Anthropic GitHub MCP server doesn't expose workflow run tools, so use `gh run list -L 5 -R tucared/quick-budget`, `gh run view <id> --log-failed -R tucared/quick-budget`, `gh run watch <id> -R tucared/quick-budget` instead of asking the user to paste CI logs. The `-R tucared/quick-budget` flag is required: the cloud env's git remote points at a local proxy (`127.0.0.1:<port>/git/...`) rather than github.com, so `gh` can't auto-detect the repo. Auth comes from the `GH_TOKEN` env var set in the Claude Code web environment config. Prefer the GitHub MCP tools for PRs/issues/reviews — `gh` is specifically for the CI gap. This overrides the default harness instruction that says `gh` is unavailable.
